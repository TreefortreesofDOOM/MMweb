...
Generated on: 2025-01-16 15:28:15
...
### Table: artwork_embeddings_chatgpt
Schema: public
...
| application_status | text | YES | 'pending'::text |  |
| application_status_updated_at | timestamp with time zone | YES |  |  |
...
- public.artwork_embeddings_chatgpt.artwork_id -> public.artworks.id
...
- Policy: Artists can view ghost profiles
  - Roles: {public}
  - Command: SELECT
  - Definition: ((EXISTS ( SELECT 1
   FROM artworks
  WHERE ((artworks.artist_id = auth.uid()) AND (artworks.id IN ( SELECT transactions.artwork_id
           FROM transactions
          WHERE ((transactions.ghost_profile_id = ghost_profiles.id) AND (transactions.status = 'succeeded'::payment_status))))))) OR (email = (( SELECT users.email
   FROM auth.users
  WHERE (users.id = auth.uid())))::text) OR (claimed_profile_id = auth.uid()) OR (is_visible = true))
...
- Policy: Users can view transactions
  - Roles: {public}
  - Command: SELECT
  - Definition: ((buyer_id = auth.uid()) OR (artist_id = auth.uid()) OR ((ghost_profile_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM auth.users
  WHERE ((users.id = auth.uid()) AND ((users.email)::text = ( SELECT ghost_profiles.email
           FROM ghost_profiles
          WHERE (ghost_profiles.id = transactions.ghost_profile_id))))))) OR ((ghost_profile_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM ghost_profiles
  WHERE ((ghost_profiles.id = transactions.ghost_profile_id) AND (ghost_profiles.claimed_profile_id = auth.uid()))))))
...
### Function: update_application_status_timestamp
...
### Function: validate_artist_role

`sql
CREATE OR REPLACE FUNCTION public.validate_artist_role()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
### Trigger: update_application_status_timestamp
- Table: profiles
- Function: EXECUTE FUNCTION update_application_status_timestamp()
- Events: UPDATE
...
### Trigger: validate_artist_role_trigger
- Table: profiles
- Function: EXECUTE FUNCTION validate_artist_role()
- Events: UPDATE, INSERT
...
### Table: artwork_embeddings_chatgpt
- artwork_embeddings_artwork_id_idx: CREATE INDEX artwork_embeddings_artwork_id_idx ON public.artwork_embeddings_chatgpt USING btree (artwork_id)
- artwork_embeddings_embedding_idx: CREATE INDEX artwork_embeddings_embedding_idx ON public.artwork_embeddings_chatgpt USING ivfflat (embedding vector_cosine_ops) WITH (lists='100')
- artwork_embeddings_pkey: CREATE UNIQUE INDEX artwork_embeddings_pkey ON public.artwork_embeddings_chatgpt USING btree (id)
- artwork_embeddings_type_idx: CREATE INDEX artwork_embeddings_type_idx ON public.artwork_embeddings_chatgpt USING btree (embedding_type)
...
- idx_transactions_ghost_profile_status: CREATE INDEX idx_transactions_ghost_profile_status ON public.transactions USING btree (ghost_profile_id, status)
